...
2、激活虚拟环境：
在cmd命令行输入D:\github\vmerp\env\Scripts\activate 或  .\env\Scripts\activate

3、在命令行中以(env) D:\github\vmerp>  开头，表明已经进入虚拟环境
创建requirement.txt,将各种依赖包名写入文件中，一行只能写一个包名
后期上线可以使用 pip install -r requirements.txt 来批量安装依赖包

4、在虚拟环境目录(env) D:\github\vmerp>  输入创建django-admin命令
(env) D:\github\vmerp>     django-admin startproject vmerp

5、进入刚才创建的项目的目录中，然后看到一个vmerp文件夹和一个manage.py文件
(env) D:\github\vmerp>cd vmerp
(env) D:\github\vmerp\vmerp>python manage.py runserver
